...
class WatchlistEntry(db.Model):
    """Represents a film a user intends to watch in the future."""
    id = db.Column(db.String(36), primary_key=True, default=generate_uuid)
    user_id = db.Column(db.String(36), db.ForeignKey("user.id"), nullable=False)
    film_id = db.Column(db.String(36), db.ForeignKey("film.id"), nullable=False)
    ...
    # Set up relationships to cleanly access properties across tables
    user_rel = db.relationship("User", backref=db.backref("watchlist_entries", lazy=True))
    film_rel = db.relationship("Film", backref=db.backref("watchlist_entries", lazy=True))
